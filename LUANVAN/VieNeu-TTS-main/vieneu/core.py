    ...
    try:
        ...
        if hasattr(codec, 'dec') and hasattr(codec.dec, 'resblocks'):
            if len(codec.dec.resblocks) > 2:
                ...
                print("   [OK] Triton compilation enabled for codec")
        return True
...
class VieNeuTTS:
    ...
    def load_lora_adapter(self, lora_repo_id: str, hf_token: str = None):
        ...
        print(f"[TARGET] Loading LoRA adapter from: {lora_repo_id}")
        ...
        try:
            ...
            print(f"   [OK] LoRA adapter loaded: {lora_repo_id}")
            return True
    ...
    def unload_lora_adapter(self):
        ...
        print(f"   [RELOAD] Unloading LoRA adapter: {self._current_lora_repo}")
        ...
        try:
            ...
            print("   [OK] LoRA adapter unloaded, original weights restored")
            return True
        except Exception as e:
    ...
    def _load_voices(self, backbone_repo, hf_token=None, clear_existing=False):
        ...
        if path_obj.exists():
            ...
            if json_path.exists():
                ...
                print(f"   [WARNING] Validation Warning: Local path '{backbone_repo}' missing 'voices.json'.")
                print(f"   [WARNING] Falling back to Custom Voice Cloning mode.")
        else:
            # Remote Repo
            if clear_existing:
                self._preset_voices.clear()
            ...
            try:
                ...
                print(f"   [WARNING] Warning: Could not load voices from repo '{backbone_repo}': {e}")
                print(f"   [WARNING] Falling back to Custom Voice Cloning mode.")
    ...
    def _load_voices_from_file(self, file_path: Path, clear_existing=False):
        ...
        try:
            ...
            if "presets" in data:
                if clear_existing:
                    self._preset_voices.clear()
                    print(f"   [CLEAN] Cleared existing voices for replacement")
                
                # Merge into existing presets
                self._preset_voices.update(data["presets"])
                print(f"   [VOICE] Loaded {len(data['presets'])} voices from {file_path.name}")
            ...
            print(f"   [WARNING] Failed to load voices from {file_path}: {e}")
    ...
    def _load_voices_from_repo(self, repo_id: str, hf_token=None):
        ...
        try:
            ...
            print(f"   [WARNING] Network check failed for voices.json. Trying local cache...")
            try:
                voices_file = hf_hub_download(
                    repo_id=repo_id,
                    filename="voices.json",
                ...
                print(f"   [OK] Using cached voices.json")
            except Exception:
        ...
        if voices_file:
            ...
            print(f"   [WARNING] Warning: Repository '{repo_id}' is missing 'voices.json'. Falling back to Custom Voice mode.")
    ...
    def infer(self, text: str, ref_audio: str | Path = None, ref_codes: np.ndarray | torch.Tensor = None, ref_text: str = None, max_chars: int = 256, silence_p: float = 0.15, crossfade_p: float = 0.0, voice: dict = None, temperature: float = 1.0, top_k: int = 50) -> np.ndarray:
        ...
        if ref_audio is not None and ref_codes is None:
            ...
            print(f"   [WARNING] No reference provided. Using default voice: {self._default_voice}")
            try:
...
class FastVieNeuTTS:
    ...
    def __init__(
        ...
        print("[OK] FastVieNeuTTS with optimizations loaded successfully!")
    ...
    def _load_voices(self, backbone_repo, hf_token=None):
        ...
        if path_obj.exists():
            ...
            if json_path.exists():
                ...
                print(f"   [WARNING] Warning: Local path '{backbone_repo}' missing 'voices.json'. Falling back to Custom Voice mode.")
        else:
            # Remote Repo
            self._load_voices_from_repo(backbone_repo, hf_token)
...
class RemoteVieNeuTTS(VieNeuTTS):
    ...
    def __init__(
        ...
        print(f"[REMOTE] RemoteVieNeuTTS ready! Using backend: {self.api_base}")
